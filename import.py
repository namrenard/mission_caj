...
def strip_accents(s:str) -> str:
    """
    Fonction pour convertir une string dans le bon format unicode.
    https://docs.python.org/3/library/unicodedata.html

    :param s: string reçue
    :return:
    """
    return ''.join(c for c in unicodedata.normalize('NFD', s) if unicodedata.category(c) != 'Mn')


def get_quizz_filename(categorie: str, titre : str, difficulte: str) -> object:
    """
     Fonction de nettoyage d'un fichier json.
     Prend en parametre une catégorie, un titre et une difficulté.

    :param categorie: str
    :param titre: str
    :param difficulte: str
    :return: un fichier en .json
    """
    return strip_accents(categorie).lower().replace(" ", "") + "_" \
           + strip_accents(titre).lower().replace(" ", "") + "_" \
           + strip_accents(difficulte).lower().replace(" ", "") + ".json"


def generate_json_file(categorie: str, titre: str, url:  str):
    """
    Fonction de génération d'un fichier json.
    Prend en paramètre une catégorie, un titre et un chemin url.
    ...
    # correction : gestion de l'erreur.
    try:
        response = requests.get(url)
    except:
        print("Erreur requête url : " + url)
    else:
        try:
            ...
            print("Erreur : Impossible de générer le questionnaire : " + titre + "\n Erreur sur les données : " + url)
